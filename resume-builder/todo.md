...
- [x] **Accessibility Audit**: Baseline was 82/100 (button-name, color-contrast, heading-order). Fixed: added `aria-label`s to ~11 icon-only buttons across `Header.tsx`, `ProfileEdit.tsx`, `ImportResumeModal.tsx`; bumped low-contrast text (`slate-500`→`slate-400` on the Login/Register OAuth divider, `gray-500`→`gray-400` on the footer copyright line); promoted `Footer.tsx`'s section headings from `h3`→`h2` to fix the `h1`→`h3` skip. Re-run scored 100/100 on `/login` and `/register`.
