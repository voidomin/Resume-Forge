...
describe("Resume Generation Service", () => {
  describe("Job Description Analysis", () => {
    it("should parse job description for keywords", () => {
      ...
      expect(mockJobDescription.toLowerCase()).toContain("node");
      expect(mockJobDescription.toLowerCase()).toContain("react");
    });
  ...
  describe("Resume Data Compilation", () => {
        ...
        experiences: [
          {
            ...
            role: mockExperience.role,
            bullets: mockExperience.bullets,
          },
        ],
      };
    ...
    it("should include education section", () => {
      const resume = {
        education: [
          {
            institution: mockEducation.institution,
            degree: mockEducation.degree,
            field: mockEducation.field,
          },
        ],
      };
  ...
  describe("ATS Scoring", () => {
    it("should calculate ATS score between 0 and 100", () => {
      const score = calculateATSScore(mockJobDescription, mockExperience);
  ...
  describe("Content Selection", () => {
    ...
    it("should prioritize skills mentioned in job description", () => {
      const profileSkills = [mockSkill];
      const jobKeywords = mockJobDescription.split(/\s+/);
...
function calculateATSScore(jobDesc: string, experience: any): number {
